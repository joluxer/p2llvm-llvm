...
Register P2TargetLowering::getRegisterByName(const char *RegName, LLT VT, const MachineFunction &MF) const {
    // MatchRegisterName() is generated by TableGen from P2RegisterInfo.td
    // (P2GenAsmMatcher.inc). It covers the complete register file: r0-r31,
    // all named special registers, c0-c463, and implicit registers.
    // Returns 0 (NoRegister) for unknown names.
    unsigned MCReg = MatchRegisterName(StringRef(RegName));

    return MCReg != 0 ? Register(MCReg) : Register();
}
