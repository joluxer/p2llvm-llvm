...
public:
  P2TargetInfo(const llvm::Triple &Triple, const TargetOptions &)
    ...
    // Type Selections
    SizeType = UnsignedLong;      // size_t = unsigned long
    IntPtrType = SignedLong;      // intptr_t = long
    PtrDiffType = SignedLong;     // ptrdiff_t = long
    WCharType = SignedInt;        // wchar_t = int (4 bytes)
    WIntType = SignedInt;         // wint_t = int
    
    // Atomic Support (32-bit)
    MaxAtomicPromoteWidth = 32;
    MaxAtomicInlineWidth = 32;
    
    // Alignment (in bits)
    SuitableAlign = 32;           // Stack Alignment: 4 Bytes
    MinGlobalAlign = 0;           // no minimal Alignment for Globals
